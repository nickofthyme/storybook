...
import { telemetry, getPrecedingUpgrade, oneWayHash } from '../telemetry';
import type { EventType } from '../telemetry';
import { loadAllPresets, cache } from '../common';
...
async function getErrorLevel({ cliOptions, presetOptions }: TelemetryOptions): Promise<ErrorLevel> {
  ...
  const presets = await loadAllPresets({
    corePresets: [require.resolve('@storybook/core-api/dist/presets/common-preset')],
    overridePresets: [],
    ...presetOptions,
  });
